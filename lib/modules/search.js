...
module.sort = -9;
module.description = `
	<p>Search for settings within LES.</p>
	...
	<div id="SearchRES-boilerplate">
		<p>You can search for LES options by module name, option name, and description.</p>
		<ul>
			<li>type a term in the search box to the left and click the magnifying glass button</li>
			<li>press <code>.</code> to open the LES console, type in <code>search <em>term</em></code>, and press Enter</li>
		</ul>
	</div>
`;
...
// ---------- View ------
export const input = once(() => downcast(string.html`<input id="SearchRES-input" type="text" placeholder="search LES settings">`, HTMLInputElement));
...
const searchResultTemplate = ({ title, category, description, moduleName, moduleID, optionName, optionKey }) => string.html`
	<div>
		<div class="SearchRES-result-header">
			...
			<span class="SearchRES-breadcrumb">LES settings console
				→ ${category}
				→ ${moduleName} (${moduleID})
...
**[${title}](${url})** -- [](#gear) [LES settings console](${settingsUrl}) > ${category} > [${moduleName}](${moduleUrl} "${moduleID}")${optionKey ? ` > [${optionKey}](${optionUrl})` : ''}
