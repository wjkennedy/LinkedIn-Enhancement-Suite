...
function mountControl() {
	...
	if (existing instanceof HTMLElement) {
		if (existing.previousElementSibling === header) return;
		existing.remove();
	}
	...
	translateToggle.addEventListener('change', () => {
		...
		Options.save(module.options.autoTranslate);
		syncControlState();
		refreshReadableContent();
	});
	...
	expandToggle.addEventListener('change', () => {
		...
		Options.save(module.options.autoExpand);
		syncControlState();
		refreshReadableContent();
	});
...
function applyCompactMediaCollapse(root: ParentNode) {
	...
	for (const media of queryElements(root, 'img, video, iframe, figure')) {
		...
		if (media.tagName === 'IMG' && media.clientWidth > 0 && media.clientHeight > 0 && media.clientWidth <= 140 && media.clientHeight <= 140) continue;
		if (media.tagName === 'FIGURE') {
			const image = media.querySelector('img');
			if (image && image.clientWidth > 0 && image.clientHeight > 0 && image.clientWidth <= 140 && image.clientHeight <= 140) continue;
		}
...
function startExtensionScanReporting() {
	if (extensionScanObserver || typeof PerformanceObserver === 'undefined') return;
...
function bindReaderColumnResizer(handle: HTMLElement, divider: number) {
	...
	handle.addEventListener('pointerdown', event => {
		...
		const finish = pointerEvent => {
			update(pointerEvent);
			Options.save(module.options.readerColumnWidths);
...
function setReaderColumnBoundary(divider: number, boundary: number, save: boolean = true) {
	...
	applyReaderColumnWidths();
	if (save) Options.save(module.options.readerColumnWidths);
}

function positionFeedComposer() {
	if (!/^\/feed(?:\/|$)/i.test(location.pathname)) return;

	const startPostControl = queryElements(document, 'button, [role="button"]').find(control =>
...
function expandReadableContent(root: ParentNode) {
	...
	if (module.options.autoTranslate.value) {
		for (const button of findTranslateButtons(root)) {
			...
			labelAutomaticTranslation(button);
			activateReadMoreButton(button);
		}
	}
}
