...
const CONTROL_SCAN_REPORT_CLASS = 'les-feed-sanitizer-scan-report';
const CONTROL_TELEMETRY_REPORT_CLASS = 'les-feed-sanitizer-telemetry-report';
...
const READER_GRID_ITEM_CLASS = 'les-reader-grid-item';
...
let extensionProbeCount = 0;
let unresolvedExtensionProbeCount = 0;
...
let readerGridObserver: ?ResizeObserver;
let readerGridFrame: ?number;
...
module.options = {
	...
	lesTVHashtag: {
		...
		description: 'Hashtag source used by LES TV. Enter it with or without the # character.',
	},
	filterPromoted: {
		type: 'boolean',
		value: true,
...
module.onSaveSettings = changedSettings => {
	...
	if (changedSettings.lesTVHashtag && document.documentElement.classList.contains(TV_VIEWER_OPEN_CLASS)) renderTVViewer();
...
function applyReaderColumnWidths() {
	...
	root.style.setProperty('--les-reader-column-one', `${first}%`);
	root.style.setProperty('--les-reader-column-two', `${second}%`);
	root.style.setProperty('--les-reader-column-three', `${third}%`);
...
function mountControl() {
	...
	const control = string.html`
		<div class="${CONTROL_CLASS}" data-les-ignore="true">
			...
			<span class="${CONTROL_SCAN_REPORT_CLASS}" role="status" aria-live="polite" title="No browser-extension probes observed in this tab.">Probes 0</span>
			<span class="${CONTROL_TELEMETRY_REPORT_CLASS}" role="status" aria-live="polite" title="No LinkedIn telemetry attempts deflected in this tab.">Deflected 0</span>
...
function syncControlState() {
	...
	syncExtensionScanReport(control);
	syncTelemetryReport(control);
...
function syncTVControl(existingControl?: Element) {
	...
	const title = tvQueue.length ?
		`Open LES TV with ${tvQueue.length} available item${tvQueue.length === 1 ? '' : 's'}` :
		'Open LES TV. It fills from visible Live, event, and video content.';
...
function reportMainWorldExtensionProbes() {
	...
	extensionProbeCount += delta;
	unresolvedExtensionProbeCount += unresolvedDelta;
	syncExtensionScanReport();
}

function reportExtensionProbe(url: string) {
	if (/^chrome-extension:\/\/invalid\//i.test(url)) {
		extensionProbeCount += 1;
		unresolvedExtensionProbeCount += 1;
		syncExtensionScanReport();
		return;
	}
...
function syncExtensionScanReport(existingControl?: Element) {
	const control = existingControl || document.querySelector(`.${CONTROL_CLASS}`);
	const report = control?.querySelector(`.${CONTROL_SCAN_REPORT_CLASS}`);
	if (!(report instanceof HTMLElement)) return;

	const count = extensionProbeCount;
	report.textContent = `Probes ${count}`;
	report.classList.toggle('les-feed-sanitizer-scan-detected', count > 0);
	report.title = count ?
		`${count} browser-extension resource probe${count === 1 ? '' : 's'} observed in this tab${unresolvedExtensionProbeCount ? ` (${unresolvedExtensionProbeCount} unresolved)` : ''}. No extension IDs are retained or transmitted by LES.` :
		'No browser-extension probes observed in this tab.';
}

function syncTelemetryReport(existingControl?: Element) {
	const control = existingControl || document.querySelector(`.${CONTROL_CLASS}`);
	const report = control?.querySelector(`.${CONTROL_TELEMETRY_REPORT_CLASS}`);
	if (!(report instanceof HTMLElement)) return;

	report.textContent = `Deflected ${telemetryDeflectedCount}`;
	report.classList.toggle('les-feed-sanitizer-telemetry-detected', telemetryDeflectedCount > 0);
	report.title = telemetryDeflectedCount ?
		`${telemetryDeflectedCount} LinkedIn telemetry attempt${telemetryDeflectedCount === 1 ? '' : 's'} deflected in this tab.` :
		'No LinkedIn telemetry attempts deflected in this tab.';
}
...
function renderTVViewer() {
	...
	if (!item) {
		...
		stage.append(string.html`<p>Browse LinkedIn Live, native videos, or Network Events to add items to this tab's LES TV queue.</p>`);
		source.hidden = true;
		register.hidden = true;
		return;
	}

	position.textContent = `${tvActiveIndex + 1} of ${tvQueue.length}`;
...
function mountReaderColumnResizers() {
	...
	for (const index of [0, 1]) {
		...
		if (!(handle instanceof HTMLElement)) {
			handle = string.html`<button class="${READER_COLUMN_RESIZER_CLASS}" type="button" data-column-divider="${index}" aria-label="Resize Reader columns" title="Drag to resize Reader columns"></button>`;
...
function layoutReaderGrid() {
	...
	const items = [];
	const seen = new Set();
	for (const post of findReaderPosts()) {
		let item = post;
		while (item.parentElement && item.parentElement !== canvas) item = item.parentElement;
		if (item.parentElement !== canvas || seen.has(item)) continue;
		seen.add(item);
		items.push(item);
	}

	for (const stale of canvas.querySelectorAll(`:scope > .${READER_GRID_ITEM_CLASS}`)) {
		if (!(stale instanceof HTMLElement) || seen.has(stale)) continue;
		resetReaderGridItem(stale);
	}

	for (const [index, item] of items.entries()) {
		item.classList.add(READER_GRID_ITEM_CLASS);
		item.style.setProperty('--les-reader-grid-column', String((index % 3) + 1));
		getReaderGridObserver()?.observe(item);
	}
	scheduleReaderGridMeasurement();
}

function getReaderGridObserver(): ?ResizeObserver {
	if (!readerGridObserver && typeof ResizeObserver !== 'undefined') {
		readerGridObserver = new ResizeObserver(entries => {
			for (const entry of entries) {
				if (entry.target instanceof HTMLElement) scheduleReaderGridMeasurement();
			}
		});
	}
	return readerGridObserver;
}

function scheduleReaderGridMeasurement() {
	if (readerGridFrame) return;
	readerGridFrame = requestAnimationFrame(() => {
		readerGridFrame = undefined;
		const measurements = [];
		for (const candidate of document.querySelectorAll(`.${READER_GRID_ITEM_CLASS}`)) {
			if (!(candidate instanceof HTMLElement) || !candidate.isConnected) continue;
			const height = candidate.getBoundingClientRect().height;
			if (!height) continue;
			measurements.push({
				item: candidate,
				span: Math.max(1, Math.ceil((height + 8) / 16)),
			});
		}
		for (const { item, span } of measurements) {
			const nextSpan = String(span);
			if (item.style.getPropertyValue('--les-reader-row-span') !== nextSpan) {
				item.style.setProperty('--les-reader-row-span', nextSpan);
			}
		}
	});
}

function clearReaderGridLayout() {
	for (const item of document.querySelectorAll(`.${READER_GRID_ITEM_CLASS}`)) {
		if (item instanceof HTMLElement) resetReaderGridItem(item);
	}
}

function resetReaderGridItem(item: HTMLElement) {
	readerGridObserver?.unobserve(item);
	item.classList.remove(READER_GRID_ITEM_CLASS);
	item.style.removeProperty('--les-reader-grid-column');
	item.style.removeProperty('--les-reader-row-span');
}
...
function performReadableContentAction(button: HTMLElement, action: 'expand' | 'translate') {
	...
	requestAnimationFrame(() => scheduleReaderGridMeasurement());
}
