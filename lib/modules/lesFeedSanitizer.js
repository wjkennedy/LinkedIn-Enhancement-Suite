...
function mountControl() {
	...
	const control = string.html`
		<div class="${CONTROL_CLASS}" data-les-ignore="true">
			${SettingsNavigation.makeUrlHashLink(module.moduleID, 'readingDensity', 'LES', CONTROL_BRAND_CLASS)}
			<label class="${CONTROL_MODE_CLASS}">
				<span>Mode</span>
				<select aria-label="LES reading density">
					<option value="compact">Compact</option>
					<option value="comfortable">Comfortable</option>
					<option value="reader">Reader</option>
				</select>
			</label>
			<label class="${CONTROL_TOGGLE_CLASS}">
				<input type="checkbox" />
	...
	if (!(densitySelect instanceof HTMLSelectElement) || !(expandToggle instanceof HTMLInputElement)) return;
	...
	addFloater(control, { container: 'visibleAfterScroll', order: 8 });
	syncControlState();
}
...
function syncControlState() {
	...
	const autoExpand = density === 'reader' || (density === 'comfortable' && module.options.expandReadMoreInComfortable.value);
	expandToggle.checked = autoExpand;
	expandToggle.disabled = density !== 'comfortable';
	expandToggle.closest('label')?.setAttribute(
		'title',
		density === 'reader'
			? 'Reader mode always expands truncated text.'
			: density === 'compact'
				? 'Auto-expand is only available in Comfortable mode.'
				: 'Expand truncated text automatically in Comfortable mode.',
	);
}
