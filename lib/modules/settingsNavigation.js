...
export function makeUrlHashLink(moduleID: string, optionKey?: string, displayText?: string, cssClass?: string): string {
	...
	if (!displayText) {
		if (mod && optionKey && mod.options[optionKey]) {
...
export function parseHash(hash: string) {
	...
	const moduleID = params && params[0] && params[0].substring(1).replace('%20', ' ') || undefined;
	const optionKey = params && params[1] && params[1].substring(1).replace('%20', ' ') || undefined;
	const mod = moduleID && Modules.getUnchecked(moduleID);

	return {
		moduleID: mod ? moduleID : undefined,
		optionKey: mod && optionKey && mod.options[optionKey] ? optionKey : undefined,
	};
}
...
function handleEmbedFailure() {
	console.warn('Embed failed. Opening LES settings console in new tab');
