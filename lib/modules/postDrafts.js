...
const COMPOSER_ACTION_PATTERN = /\b(?:emoji|enhance post|add media|add a photo|photo|gif|more|celebrat|expert|sticker)\b/i;
...
const COMPOSER_ROOT_SELECTOR = '.share-box, .share-creation-state, .comments-comment-box, [class*="comment-box" i], [role="dialog"], .artdeco-modal';
...
function findComposerActionRow(editor: HTMLElement): ?HTMLElement {
	...
	const preferred = controls.find(control => /emoji|enhance post|add media|photo|gif|sticker/i.test(controlText(control))) || controls[0];
	if (!(preferred instanceof HTMLElement)) {
		const commentRoot = editor.closest('.comments-comment-box, [class*="comment-box" i]');
...
function findComposerContainer(editor: HTMLElement): ?HTMLElement {
	const explicitRoot = editor.closest('.share-creation-state, .share-box, .comments-comment-box, [class*="comment-box" i]');
...
function hasComposerControls(container: HTMLElement): boolean {
	...
	const hasCommentButton = controls.some(control => /\bcomment\b/i.test(controlText(control)));
	const accessibleName = [
		container.getAttribute('aria-label') || '',
		container.getAttribute('data-testid') || '',
		container.textContent || '',
	].join(' ');
	return hasPostButton || hasCommentButton || /\b(?:create|share|write).{0,20}post\b/i.test(accessibleName);
}
...
function mountManager() {
	if (manager) return;
	manager = string.html`
		<dialog class="${MANAGER_CLASS}" data-les-ignore="true" data-editor-view="split" aria-label="LES post drafts" hidden>
			<section>
				<header>
	...
	// LinkedIn's post composer is itself a modal. Keep its focus and keyboard
	// handlers from consuming events raised inside the LES draft editor.
	for (const eventName of ['focusin', 'focusout', 'keydown', 'keyup', 'input', 'change', 'click']) {
	...
	manager.addEventListener('cancel', (event: Event) => {
...
function openManager(editor: ?HTMLElement) {
	...
	// Keep the manager at the document root. LinkedIn's share modal applies
	// broad descendant styles and stacking/overflow rules that can strip the
	// manager's layout when it is mounted inside that modal.
...
function flushManagerDraft(showStatus: boolean = false) {
	...
	if (!draft || !(title instanceof HTMLInputElement)) return;

	draft.body = body instanceof HTMLTextAreaElement ? body.value : draft.body;
	draft.title = title.value.trim() || titleFromBody(draft.body);
	draft.updatedAt = Date.now();
	persistDrafts();
...
async function useActiveDraft() {
	...
	const draft = drafts.find(item => item.id === activeDraftId);
	if (!draft) return;
	// A modal dialog makes the rest of the page inert. Release the top layer
	// while copying into LinkedIn, but leave the manager rendered so it can be
	// restored with an error message if LinkedIn rejects the update.
	dismissManagerModal();
	const content = markdownToComposerContent(draft.body);
	const copied = await copyDraftContent(content);
