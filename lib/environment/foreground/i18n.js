...
export let locale = navigator.language || 'en';
...
function getDocumentLocale() {
	const documentLocale = typeof document !== 'undefined' && document.documentElement.getAttribute('lang');
	const normalized = documentLocale && documentLocale.toLowerCase().replace('_', '-');

	return normalized && isValidLocale(normalized) ? normalized : locale;
}
...
export async function _loadI18n(): Promise<void> {
	...
	if (documentLocale) locale = documentLocale;

	// fast path: i18n dictionary is cached
	// should be hit almost every time
	if (
		localStorage.getItem(CACHED_LANG_KEY) === documentLocale &&
	...
	messages = await sendMessage('i18n', documentLocale);
	...
	try { // Fails if remaining localStorage space is insufficient
		...
		localStorage.setItem(CACHED_LANG_KEY, documentLocale);
		localStorage.setItem(CACHED_MESSAGES_TOKEN_KEY, buildToken);
	} catch (e) {
		console.error('Could not cache i18n - LES will load slowly', e);
	}
}
