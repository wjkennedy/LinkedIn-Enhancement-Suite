...
export function start() {
	...
	// Update all relative links to refer to the page where the console was opened.
...
function load(moduleID, optionKey) {
	...
	const validOptionKey = optionKey && mod.options.hasOwnProperty(optionKey) ? optionKey : undefined;
	...
	if (validOptionKey) {
		highlightOption(mod, validOptionKey);
	}

	if (mod === Search.module) {
		Search.search(validOptionKey);
		requestAnimationFrame(() => Search.input().focus());
	} else {
		Search.input().blur();
	}

	SettingsNavigation.setHash(SettingsNavigation.makeUrlHash(mod.moduleID, validOptionKey));
}

function getDefaultModule() {
	const mod = Modules.all().find(module => !module.hidden);
	if (!mod) throw new Error('No visible modules are available for the settings console.');
	return mod;
}
...
const createKeyCodeModal = once(() => {
	...
	window.addEventListener('keydown', e => {
		if (captureKey && ![NAMED_KEYS.Shift, NAMED_KEYS.Control, NAMED_KEYS.Alt].includes(e.key)) {
			...
			// Avoid global ID collisions on host pages.
...
function drawOptionInput(mod, optionName, optionObject, isTable) {
	...
	switch (optionObject.type) {
		...
		case 'hidden':
			...
			if (optionObject.type === 'text' && optionObject.readOnly) {
...
const abandonChangesConfirmation = 'Abandon your changes to LES settings?';
